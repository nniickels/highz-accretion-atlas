"""Render the canonical per-object seed-mass/seed-redshift gallery."""
...
def panel_path(version: str, obj: pd.Series) -> Path:
    object_class = atlas.slug(obj["object_class"]).replace("-", "_")
    stem = atlas.slug(obj["physical_object_id"])
    return (
        ROOT / "results" / version / "gallery" / "per_object" / object_class
        / "seed_redshift_maps" / f"{version}_seed_redshift_map_{stem}.png"
    )


def plot_seed_redshift_map(version: str, obj: pd.Series, output: Path) -> None:
...
def materialize(version: str, objects: pd.DataFrame, *, rebuild: bool) -> pd.DataFrame:
    ...
    for _, obj in objects.sort_values(["object_class", "redshift", "physical_object_id"]).iterrows():
        ...
        if rebuild or not output.exists():
            if eligible:
                plot_seed_redshift_map(version, obj, output)
            else:
                atlas.VERSION = version
                atlas.status_panel(obj, "seed_redshift_map", output)
        rows.append({
            "release": version,
            ...
            "product_kind": "seed_redshift_map",
    ...
    if len(result) != len(objects) or result["physical_object_id"].nunique() != len(objects):
        raise AssertionError("Seed-redshift gallery must contain one panel per object")
    return result
