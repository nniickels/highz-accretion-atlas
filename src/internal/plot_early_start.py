...
def render(destination=None):
    ...
    shift = models.growth_log10_factor(1, .1, cosmic_time_gyr(30)-cosmic_time_gyr(3400))
    def mass_track(seed, start):
        return seed + models.growth_log10_factor(1, .1, cosmic_time_gyr(z)-cosmic_time_gyr(start))
    with plt.rc_context({**plt.rcParamsDefault, 'font.family': 'STIXGeneral',
        ...
        for i, (ax, start) in enumerate(zip(axes, (30, 3400))):
            for seed, color, style in SEEDS:
                mass = mass_track(seed, start)
                if start == 3400:
                    np.testing.assert_allclose(mass-mass_track(seed, 30), shift, atol=1e-12)
        ...
        fig.text(.5,.035,f'Radiation included; earlier onset adds {shift:.3f} dex (a factor of {10**shift:.2f}).',ha='center')
