"""Build the immutable, catalogue-only v7 GNIRS-50 extension."""
...
from src.internal.compatibility.v7_shen19 import SOURCE_KEY, build_shen19_admission, build_shen19_observables
...
def build_v7_gnirs_catalogues(
    ...
    """Return v7 products without modifying any frozen earlier artifact."""
    ...
    if len(batch.identity_candidates) != EXPECTED_REVIEWED_IDENTITIES:
        raise ValueError("v7 must preserve six reviewed GNIRS/XQR identity decisions")
    if not batch.identity_candidates["decision"].eq("accepted").all():
        raise ValueError("v7 GNIRS/XQR identity decisions must all be accepted")
    ...
    measurements.loc[repeated_new, "preferred_measurement_reason"] = (
        "v7 XQR-30 measurement retained as preferred; GNIRS measurement preserved as alternate"
    )
    measurements.loc[repeated_new, "match_method"] = "reviewed literature identity"
    measurements.loc[repeated_new, "match_reference"] = "v7 reviewed identity registry"
    ...
    measurements.loc[is_new & ~repeated_new, "match_reference"] = (
        "no v7 candidate within 0.5 arcsec and delta-z 0.01 and no manual identity assertion"
    )
