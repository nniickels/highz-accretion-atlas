"""Build the v7 provenance and object-evidence-policy catalogue layer."""
...
from src.internal.compatibility.v7_core_catalogue import _aggregate_objects, _build_host_systems, _build_strata, _set_eligibility
...
def _aggregate_objects_with_preferred_evidence(measurements: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(preferred) != measurements["physical_object_id"].nunique():
        raise ValueError("v7 requires exactly one preferred measurement per object")
...
def finalize_v7_catalogues(
    ...
    correction = build_scholtz_admission(
        scholtz_correction,
        template_columns=list(v7_measurements.columns),
        reserved_ids=set(v7_measurements["physical_object_id"]),
        source_validator=validate_scholtz_99671_correction,
        catalogue_release=CATALOGUE_RELEASE,
        project_version="v7",
    )
    measurements = pd.concat(
        [v7_measurements, correction.dropna(axis=1, how="all")], ignore_index=True,
    )
    ...
    observables = pd.concat([v7_observables, correction_observables], ignore_index=True)
    observables["catalogue_release"] = CATALOGUE_RELEASE
    validate_v7_observables(observables, measurements["measurement_id"])
    candidates = v7_reviewed_candidates.copy()
    ...
    if set(measurements.loc[measurements["source_key"].eq(SOURCE_KEY), "object_id"]) != set(
        ...
        raise ValueError("v7 Scholtz catalogue membership changed")
    return {
