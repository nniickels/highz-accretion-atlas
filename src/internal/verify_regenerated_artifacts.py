...
CSV values use the shared cross-platform numeric tolerance; PNGs require identical
RGB pixels (encoding metadata may differ). The default baseline is Git HEAD,
never the just-regenerated files. CI archive checkouts use HIGHZ_BASELINE_ROOT.
"""
...
def compare_artifact(expected: Path, actual: Path) -> None:
        ...
        try:
            ...
            with Image.open(expected) as left, Image.open(actual) as right:
                ...
                for y in range(0, left.height, 256):
                    ...
                    if ImageChops.difference(left_strip, right_strip).convert('RGB').getbbox() is not None:
                        raise AssertionError(f'{actual}: rendered pixels differ')
...
def verify_against(baseline: Path, generated: Path) -> int:
    def paths(root):
    ...
    for name in sorted(expected):
        compare_artifact(baseline / name, generated / name)
    return len(expected)
...
def main() -> None:
    ...
    if args.baseline_root:
        ...
        count = verify_against(baseline, ROOT)
    else:
        with git_baseline(ROOT) as baseline:
            count = verify_against(baseline, ROOT)
    print(f'Verified {count} regenerated artifacts against independent baseline; CSV values and PNG pixels agree')
