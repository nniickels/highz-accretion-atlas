"""Build the immutable, catalogue-only v7.1 XQR-30 extension."""
...
from src.v7_xqr30 import SOURCE_KEY, build_xqr30_admission, build_xqr30_observables


CATALOGUE_RELEASE = "v7.1-accreting-atlas-catalogue"
...
def build_v7_1_catalogues(
    ...
    """Return v7.1 products without modifying any frozen v7.0 artifact."""
    ...
    if not batch.identity_candidates.empty:
        raise ValueError("XQR-30 must have no coordinate/redshift candidate in the v7.0 atlas")
    ...
    if len(measurements) != 161 or measurements["physical_object_id"].nunique() != 154:
        raise ValueError("v7.1 must contain 161 measurements and 154 physical objects")
    preferred = measurements.groupby("physical_object_id")["preferred_measurement_flag"].sum()
    if not preferred.eq(1).all():
        raise ValueError("v7.1 must retain one preferred measurement per physical object")
    ...
    if len(objects) != 154 or len(hosts) != 153:
        raise ValueError("v7.1 must contain 154 objects and 153 host systems")
