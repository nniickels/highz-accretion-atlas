"""Build the immutable catalogue-only v7.4 JADES narrow-line extension."""
...
def build_v7_4_catalogues(
    v7_3_measurements: pd.DataFrame,
    v7_3_observables: pd.DataFrame,
    v7_3_aliases: pd.DataFrame,
    scholtz_source: pd.DataFrame,
    identity_overrides: pd.DataFrame,
) -> dict[str, pd.DataFrame]:
    new = build_scholtz_admission(
        scholtz_source, template_columns=list(v7_3_measurements.columns),
        reserved_ids=set(v7_3_measurements["physical_object_id"].astype(str)),
    )
    new_observables = build_scholtz_observables(scholtz_source)
    batch = assemble_source_family_batch(
        v7_3_measurements.copy(),
    ...
    if not preferred.eq(1).all():
        raise ValueError("v7.4 must retain one preferred measurement per physical object")
    ...
    if observed != (233, 218, 217):
        raise ValueError(f"v7.4 cardinality mismatch: {observed}")
    ...
    prior_observables = v7_3_observables.copy()
