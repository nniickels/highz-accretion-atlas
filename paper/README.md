...
The two-object seed-mass/starting-redshift comparison is regenerated by
`.venv/bin/python -m src.internal.plot_seed_timing_comparison` and notebook 02.
It uses the catalogue central masses, fixed efficiency 0.1 and no merger boost.
Its heatmaps are rasterized within the PDF; labels and contours are vector elements.
