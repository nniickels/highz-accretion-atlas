...
at $z\geq4$. The catalogue contains 320 literature measurements representing 311
physical objects in 310 host systems. Its science layer uses 219 measurements
and 212 preferred physical-object rows with canonical numeric black-hole masses;
99 catalogue objects without such masses remain explicit exclusions. The atlas
...
\midrule
Measurements / physical objects / host systems & 320 / 311 / 310 \\
Source-local observables & 675 \\
Growth-eligible measurements / objects & 219 / 212 \\
Primary measurements / objects & 209 / 202 \\
Broad-line eligible objects & 211 \\
Catalogue-only physical objects & 99 \\
Alternate-measurement sensitivity pairs & 7 \\
Follow-up rows / ranked rows & 311 / 212 \\
Source-family caveat rows & 29 \\
Per-object $f_{\rm Edd}$--mass/seed-redshift--mass panels & 622 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Current catalogue landscape. Left: all 212 growth-eligible physical
objects in black-hole-mass--redshift space. Right: growth-product coverage for
all 311 objects. The 99 catalogue-only objects remain available for provenance
...
\caption{Complete v3 growth-track view. All 212 objects with supported
canonical black-hole masses are plotted against reference tracks. The lower
panel retains the redshift locations of all 99 catalogue-only objects for which
...
\caption{Compatibility summary using every one of the 212 growth-computable
...
matrix, including all 99 unavailable objects, is included in the supplement.}
...
auditable. The follow-up matrix retains all 311 objects, ranks the 212
growth-eligible objects, and leaves the other 99 visibly unranked. A separate
29-row caveat table gives one compact provenance and interpretation summary for
every admitted source family.

\begin{figure}[p]
\centering
\includegraphics[width=0.99\textwidth]{../results/v3/figures/v3_monte_carlo_summary.png}
\caption{Complete Monte Carlo uncertainty summary. Every one of the 212
numerical posteriors is plotted; point size encodes the 16th--84th percentile
width. The adjacent accounting panel retains all 99 catalogue-only objects.
...
gallery. Every one of the 311 objects has an $f_{\rm Edd}$--mass panel and a
seed-redshift--mass panel under the final gallery path. For the 99
...
the same dataset version. It provides reproducible observational triage across 311
