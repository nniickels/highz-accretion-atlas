...
at $z\geq4$. The catalogue contains 258 literature measurements representing 249
physical objects in 248 host systems. Its science layer uses 160 measurements
and 153 preferred physical-object rows with canonical numeric black-hole masses;
96 catalogue objects without such masses remain explicit exclusions. The atlas
...
The expanded heterogeneous layer contributes 75 identity-audited objects:
...
narrow-line LRDs, GHZ2, and eight compact blue broad-line emitters. None has an
...
\midrule
Measurements / physical objects / host systems & 258 / 249 / 248 \\
Source-local observables & 415 \\
Growth-eligible measurements / objects & 160 / 153 \\
Primary measurements / objects & 152 / 145 \\
Broad-line eligible objects & 152 \\
Catalogue-only physical objects & 96 \\
Alternate-measurement sensitivity pairs & 7 \\
Follow-up rows / ranked rows & 249 / 153 \\
Source-family caveat rows & 25 \\
Per-object $f_{\rm Edd}$--mass/seed-redshift--mass panels & 498 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Current catalogue landscape. Left: all 153 growth-eligible physical
objects in black-hole-mass--redshift space. Right: growth-product coverage for
all 249 objects. The 96 catalogue-only objects remain available for provenance
...
\caption{Complete v3 growth-track view. All 153 objects with supported
canonical black-hole masses are plotted against reference tracks. The lower
panel retains the redshift locations of all 96 catalogue-only objects for which
...
\caption{Compatibility summary using every one of the 153 growth-computable
...
matrix, including all 96 unavailable objects, is included in the supplement.}
...
auditable. The follow-up matrix retains all 249 objects, ranks the 153
growth-eligible objects, and leaves the other 96 visibly unranked. A separate
25-row caveat table gives one compact provenance and interpretation summary for
every admitted source family.

\begin{figure}[p]
\centering
\includegraphics[width=0.99\textwidth]{../results/v3/figures/v3_monte_carlo_summary.png}
\caption{Complete Monte Carlo uncertainty summary. Every one of the 153
numerical posteriors is plotted; point size encodes the 16th--84th percentile
width. The adjacent accounting panel retains all 96 catalogue-only objects.
...
gallery. Every one of the 249 objects has an $f_{\rm Edd}$--mass panel and a
seed-redshift--mass panel under the final gallery path. For the 96
...
the same dataset version. It provides reproducible observational triage across 249
