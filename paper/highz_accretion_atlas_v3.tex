...
original figure dimensions and margins. Broad-line AGN are purple and luminous
quasars are yellow for direct visual contrast.}
\label{fig:allgrowth}
\end{figure}
...
points retain the companion's original blue/orange palette. In the no-mass
panel, narrow-line candidates are blue, luminous quasars are orange, and the
X-ray candidate is gray. This denser view is an assumption comparison,
...
{uncertainty-filtered companion} retains the same 72 curves while excluding
only the four luminous quasars whose maximum reported black-hole-mass
uncertainty exceeds 0.7 dex. The exact selection is preserved in
\href{../results/v3/tables/v3_growth_track_uncertainty_filter.csv}
{the filter audit table}; all other numerical objects and all 23 no-inference
markers remain visible. This is a display-specific sensitivity view and does
not alter the canonical catalogue or rankings.
...
historical v1 reference curves for direct comparison with v3, while its
uncertainty-filtered companion makes the dense comparison readable without the
four luminous-quasar error-bar outliers.
