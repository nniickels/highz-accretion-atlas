...
\begin{abstract}
We present the v3 provenance-aware atlas of JWST-identified accreting black holes
at $z\geq4$. The catalogue contains 350 literature measurements representing 340
physical objects in 339 host systems. Its science layer uses 244 measurements
and 237 preferred physical-object rows with canonical numeric black-hole masses;
103 catalogue objects without such masses remain explicit exclusions. The atlas
...
UNCOVER-20466 has the highest point-estimate growth pressure. These diagnostics
...
The atlas asks which JWST-identified objects place the strongest pressure on simple
...
baccus2026,fei2026,zhuang2025,lin2025},
host-selected broad-line candidates \cite{ren2025}, an X-ray evidence history
\cite{bogdan2024,zou2026}, and JADES narrow/high-ionization candidates
\cite{scholtz2025}, and GN-z11 high-ionization-line evidence \cite{maiolino2024}.
The heterogeneous extension adds high-ionization and narrow-line candidates
\cite{chisholm2024,tang2025,mazzolari2024,zhang2026,chavezortiz2026},
MIRI SED-selected candidates \cite{leung2026,lyu2024}, the X-ray/UV source GHZ9
\cite{napolitano2025}, and compact blue broad-line-emitter candidates
\cite{mascia2026}, additional UNCOVER high-ionization candidates
\cite{treiber2025}, and the gas-enshrouded broad-Hbeta source MoM-BH*-1
\cite{naidu2026}. The final completion also adds the GHZ4 and GHZ7
high-ionization candidates \cite{napolitano2024}.
Selection functions, evidence strength, and mass estimators
...
The expanded heterogeneous layer contributes 78 identity-audited objects:
GN-42437, three Tang high-ionization candidates, 25 CEERS narrow-line
candidates, 12 MEOW and 19 SMILES MIRI-selected candidates, GHZ9, five
narrow-line LRDs, GHZ2, eight compact blue broad-line emitters, two additional
UNCOVER high-ionization components, and MoM-BH*-1. None has an
unconditional source-published black-hole mass suitable for the current growth
calculation. Method-dependent ranges, upper limits, and masses derived by
assuming an Eddington ratio remain source-local contextual observables rather
than canonical growth anchors.

The final v3 completion adds 15 NEXUS NIRCam/WFSS broad-line measurements
\cite{zhuang2025}, 13 COSMOS-3D NIRCam-grism broad-line AGN
\cite{lin2025}, and the GHZ4/GHZ7 high-ionization candidates
\cite{napolitano2024}. One NEXUS measurement is identity-linked to an existing
compact-blue-emitter row, giving 30 measurements, 29 new physical objects, and
25 new growth-plottable objects.
...
Broad-line eligible objects & 236 \\
Catalogue-only physical objects & 103 \\
Alternate-measurement sensitivity pairs & 7 \\
Follow-up rows / ranked rows & 340 / 237 \\
Source-family caveat rows & 32 \\
Per-object $f_{\rm Edd}$--mass/seed-redshift--mass panels & 680 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Current catalogue landscape. Left: all 237 growth-eligible physical
objects in black-hole-mass--redshift space. Right: growth-product coverage for
all 340 objects. The 103 catalogue-only objects remain available for provenance
...
\caption{Complete v3 growth-track view. All 237 objects with supported
canonical black-hole masses are plotted against reference tracks. The lower
panel retains the redshift locations of all 103 catalogue-only objects for which
a numerical growth history cannot be inferred. Thus every physical object is
visible without assigning unsupported masses. The common reversed redshift
axis spans $z=12$ to $z=3$, centering the observed sample while retaining the
original figure dimensions and margins. Broad-line AGN are purple.}
\label{fig:allgrowth}
\end{figure}
...
for 72 reference curves in total. Seed mass is encoded by color, $\fedd$ by
line style, efficiency by line width, and merger boost by opacity; numerical
points retain the companion's original blue palette. In the no-mass
panel, narrow-line candidates are blue and the X-ray candidate is gray. This denser view is an assumption comparison,
...
\caption{Descriptive growth-pressure distributions for the JWST broad-line
AGN sample. Boxes and points summarize
...
\caption{Compatibility summary using every one of the 237 growth-computable
...
matrix, including all 103 unavailable objects, is included in the supplement.}
\label{fig:compatibility}
\end{figure}

\section{Current results}

UNCOVER-20466 has the highest point-estimate reference growth pressure, requiring
$\fedd=1.452$ for a $10^2\,M_\odot$ seed. It also leads the uncertainty-aware
navigation view, with a median required $\fedd=1.453$.
These results express tension with that reference
...
contains 7 comparisons; it shows how retained measurement choice changes
$\log_{10}\mbh$ and required $\fedd$ while leaving the canonical measurement choice
auditable. The follow-up matrix retains all 340 objects, ranks the 237
growth-eligible objects, and leaves the other 103 visibly unranked. A separate
32-row caveat table gives one compact provenance and interpretation summary for
every admitted source family.

\begin{figure}[p]
\centering
\includegraphics[width=0.99\textwidth]{../results/v3/figures/v3_monte_carlo_summary.png}
\caption{Complete Monte Carlo uncertainty summary. Every one of the 237
numerical posteriors is plotted; point size encodes the 16th--84th percentile
width. The adjacent accounting panel retains all 103 catalogue-only objects.
...
\caption{Sensitivity to 7 retained alternate measurements. Numbered points are
...
gallery. Every one of the 340 objects has an $f_{\rm Edd}$--mass panel and a
seed-redshift--mass panel under the final gallery path. For the 103
...
\item Narrow-line, high-ionization, X-ray, MIRI-selected, and broad-line-emitter
candidates without canonical numeric masses are valuable evidence records but
cannot enter the current growth ranking.
...
the same dataset version. It provides reproducible observational triage across 340
JWST-identified objects while keeping unsupported demographic and formation-channel claims
