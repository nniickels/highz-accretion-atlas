...
at $z\geq4$. The catalogue contains 183 literature measurements representing 174
physical objects in 173 host systems. Its science layer uses 160 measurements
and 153 preferred physical-object rows with canonical numeric black-hole masses;
21 catalogue objects without such masses remain explicit exclusions. The atlas
...
\midrule
Measurements / physical objects / host systems & 183 / 174 / 173 \\
Source-local observables & 183 \\
Growth-eligible measurements / objects & 160 / 153 \\
Primary measurements / objects & 152 / 145 \\
Broad-line eligible objects & 152 \\
Catalogue-only physical objects & 21 \\
Alternate-measurement sensitivity pairs & 7 \\
Follow-up rows / ranked rows & 174 / 153 \\
Source-family caveat rows & 16 \\
Per-object $f_{\rm Edd}$--mass/seed-redshift--mass panels & 348 \\
\bottomrule
\end{tabular}
\end{table}
...
all 174 objects. The 21 catalogue-only objects remain available for provenance
...
panel retains the redshift locations of all 21 catalogue-only objects for which
...
axis spans $z=10$ to $z=3$, centering the observed sample while retaining the
...
matrix, including all 21 unavailable objects, is included in the supplement.}
...
auditable. The follow-up matrix retains all 174 objects, ranks the 153
growth-eligible objects, and leaves the other 21 visibly unranked. A separate
16-row caveat table gives one compact provenance and interpretation summary for
...
width. The adjacent accounting panel retains all 21 catalogue-only objects.
...
gallery. Every one of the 174 objects has an $f_{\rm Edd}$--mass panel and a
seed-redshift--mass panel under the final gallery path. For the 21
...
the same dataset version. It provides reproducible observational triage across 174
