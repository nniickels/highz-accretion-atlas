...
Current status: see [the reassessment after priority revisions](manuscript-reassessment-2026-09-11.md). This file preserves the earlier reviewed snapshot and subsequent change record.
...
## Implemented revisions — 11 September 2026

The reviewed snapshot above is retained as the assessment record. The following revisions were subsequently made in the repository manuscript and Overleaf:

- Limited the Dayal parameter attribution to H0 and Ωm; stated explicitly that this calculation sets ΩΛ by spatial flatness.
- Replaced the overly broad exclusion statement with the recorded treatment of seven conditional masses, cited their sources, and explained why COSMOS3D-13852 remains included with a separate scattering test.
- Added calibration-error markers and explanatory notes directly below Table 1. The notes distinguish the separately reported scales for RUBIES-EGS-55604, GN-38509 and J0910_2028_12910 from the scatter already included for ZS7.
- Added the downward mass shift needed to reach a required average Eddington ratio of one to all 12 rows of Table 1 and to the generated analysis CSV. The text compares these shifts with the reported systematic scales for selected objects.
- Used the required time-average notation in the probability expression and rewrote the conclusions with explicit growth assumptions and Monte Carlo criteria.
- Corrected the lowercase sentence opening in Appendix D.4 and shortened the data-path sentences, eliminating the existing LaTeX overfull-line warning.

The sample membership, adopted masses, existing growth results and probabilities are unchanged. The additional mass-shift calculation is generated by the publication workflow and tested by recovering a required ratio of one and checking values on either side. The complete test suite passed with 99 tests. No new source exclusions, spin model, primordial-growth model, priority claim or journal-specific formatting were introduced.
