...
\begin{table}[ht]
\centering
\caption{Current v5 physical-object growth-pressure leaders under the baseline reference.}
\label{tab:v5leaders}
\small
\begin{tabular}{@{}lclrr@{}}
\toprule
Point rank & Object & Preferred source & $f_{\rm Edd,req}^{100}$ & $\log M_{\rm seed,req}^{0.3}$ \\
\midrule
1 & GN-38509 & JADES & 1.065 & 6.719 \\
2 & GS-20057765 & JADES & 1.355 & 6.150 \\
3 & RUBIES-EGS-49140 & Taylor & 1.016 & 6.411 \\
4 & CEERS-00717 & Harikane & 1.027 & 6.242 \\
5 & GOODS-N-9771 & Matthee & 0.810 & 6.124 \\
\bottomrule
\end{tabular}
\end{table}

The uncertainty-aware first through third positions are GN-38509, GS-20057765, and RUBIES-EGS-49140. GOODS-N-9771 is fourth and CEERS-00717 is fifth. The swap in fourth and fifth place illustrates the intended use of the Monte Carlo layer: point rank and uncertainty rank answer related but not identical questions. The table is a growth-pressure ordering under the declared reference assumptions, not an evidence-strength ordering or a population statement.
...
\begin{enumerate}
  ...
  \item \repo{scripts/generate\_v5\_blagn\_science.py} and \repo{src/v5\_science.py}: current measurement/object evaluation, rankings, uncertainty, summaries, and alternate-measurement sensitivity.\par
...
Continuous integration installs the pinned Python 3.12 environment, runs the complete regression suite, verifies every manifest hash, rebuilds all 18 frozen v4 catalogue/science CSVs in memory at 10,000 draws with seed 20260808, and requires the checkout to remain clean. These checks protect implementation behavior, release byte anchors, source counts and identity links, explicit review decisions, confidence semantics, systematic separation, measurement sensitivity, and current numerical rankings; they do not establish that the astrophysical assumptions are uniquely correct.
...
The completed v1--v5 work demonstrates the atlas concept: provenance-aware ingestion, validated standardization, explicit reviewed measurement/object identity, transparent physical modeling, point and Monte Carlo rankings, selection-aware summaries, duplicate-measurement sensitivity, class-aware scaffolding, release-specific visualization, and automated regression checks. v5 adds five new objects and five independent measurements of existing CEERS objects while preserving every literature row and preventing double-counting. This is not evidence that one formation pathway is required. The next phase should finish manuscript consolidation, establish class-specific eligibility rules, and then add more realistic accretion histories.
