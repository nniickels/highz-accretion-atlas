...
The project begins with one relatively homogeneous class from one source paper: the JADES broad-line AGN catalogue of Juod\v{z}balis et al.\ (2025)~\cite{juodzbalis2025}. The source registry records the paper, tables used, selection, survey, and inference methods. The raw table contains 34 measurements; the reproducible standardization pass applies the $z\geq4$ selection and produces 23 measurement rows spanning $z=4.133$--$8.913$. The current quality classification contains 18 robust and 5 tentative measurements.
...
\begin{enumerate}
  \item \repo{data/sources.md} and \repo{data/raw/v1\_raw.csv}: source provenance and extracted measurements.
  \item \repo{docs/catalogue-schema.md}: canonical fields, validation rules, and missing-value policy.
  \item \repo{src/standardize\_data.py} and \repo{scripts/process\_data.py}: raw-to-processed catalogue construction.
  \item \repo{src/models.py}: cosmology and black-hole growth calculations.
  \item \repo{src/scoring.py} and \repo{scripts/v1\_evaluate.ipynb}: scenario evaluation and exploratory compatibility products.
  \item \repo{scripts/generate\_v1\_rankings.py}: point-estimate physical-pressure and follow-up rankings.
  \item \repo{scripts/}\repo{generate\_v1\_uncertainty\_rankings.py}: deterministic Monte Carlo summaries and probability-based rankings.
  \item \repo{scripts/}\repo{generate\_v1\_final\_figures.py}: publication-style figures driven by the result tables.
  \item \repo{tests/test\_v1\_pipeline.py}: model, schema, scoring, ranking, uncertainty, and numerical regression checks.
\end{enumerate}
...
\begin{itemize}
  \item v1 contains one source paper and one broad-line AGN class, so it does not yet measure cross-survey or cross-method population behavior;
  \item the main growth diagnostics assume constant lifetime-average accretion rather than explicit duty cycles or bursts;
  \item the split-normal mass model approximates reported asymmetric uncertainties but is not a reconstruction of each source's full posterior;
  \item the $\pm0.3$ dex cases are transparent sensitivity tests, not a complete virial-mass systematic model; and
  \item measurement rows are not yet separated from stable physical-object identities across papers.
\end{itemize}
