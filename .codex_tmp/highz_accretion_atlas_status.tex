...
\large Project Status Through the v7 Catalogue Layer}
...
\begin{abstract}
JWST has identified accreting black-hole candidates at epochs where the available growth time is short and inferred physical quantities remain method-dependent. This project develops a reproducible, source-tracked, uncertainty-aware atlas that ranks observational leverage under explicit seed and accretion assumptions. Project v1 standardized 23 JADES broad-line AGN measurements at $z\geq4$; v2 added point and Monte Carlo rankings plus figure prototypes; v3 added 37 Taylor et al.\ CEERS/RUBIES measurements; v4 added 20 Matthee et al.\ EIGER/FRESCO and 16 Lin et al.\ ASPIRE measurements; v5 added all ten Harikane et al.\ NIRSpec measurements; v6 added six $z\geq4$ Davis/THRILS measurements; and the catalogue-only v7 layer adds seven evidence-graded Ren et al.\ candidate nuclei in six host systems. The current catalogue has 119 measurements representing 112 physical objects in 111 host systems, while v6 remains the completed science release. The v7 primary eligibility tier contains 112 measurements and 105 objects; six BLR-conditional Ren masses remain exploratory only. Reported statistical errors and source-specific virial systematics remain separate. A new source-family batch gate now permits larger coherent additions while enforcing source-local observables, standardized compatibility, and explicit identity review. XQR-30 is selected as the next separate luminous-quasar comparison batch. Under the baseline $z_{\rm seed}=30$, $\epsilon=0.1$, and no-merger reference, existing rankings express observational growth pressure and do not prove a unique seed or accretion channel. No pooled v7 ranking or figure is yet claimed.
\end{abstract}
...
\begin{figure}[p]
\centering
\includegraphics[width=0.91\textwidth]{../results/releases/v2/figures/main_text/v2_main_text_mbh_redshift_growth_overview.png}
...
\begin{figure}[p]
\centering
\includegraphics[width=0.99\textwidth]{../results/releases/v2/figures/main_text/v2_main_text_uncertainty_forest.png}
...
\begin{figure}[p]
\centering
\includegraphics[width=0.94\textwidth]{../results/releases/v5/figures/main_text/v5_main_text_mbh_redshift_growth_overview.png}
...
\begin{figure}[p]
\centering
\includegraphics[width=0.98\textwidth]{../results/releases/v5/figures/main_text/v5_main_text_accretion_history_diagnostics.png}
...
\begin{figure}[p]
\centering
\includegraphics[width=0.94\textwidth]{../results/releases/v5/figures/main_text/v5_appendix_measurement_choice_sensitivity.png}
...
\begin{figure}[p]
\centering
\includegraphics[width=0.98\textwidth]{../results/releases/v5/figures/main_text/v5_main_text_primary_vs_full_ranking.png}
...
Continuous integration installs the pinned Python 3.12 core environment, runs the complete regression suite, verifies exact manifest membership and every manifest hash, rebuilds the frozen v4 and v5 releases, all 21 current v6 catalogue/science CSVs, and the catalogue-only v7 products in memory, and requires the checkout to remain clean. These checks protect implementation behavior, release byte anchors and counts, source-local foreign keys, standardized compatibility, identity links, explicit review decisions, confidence semantics, systematic separation, measurement sensitivity, and current numerical rankings; they do not establish that the astrophysical assumptions are uniquely correct.
...
\subsection*{1. Audit the first larger source-family batch}
Extract the complete XQR-30 source table for its 42 luminous $z>6$ quasars, retain Mg\,II as the paper's primary mass comparison and C\,IV quantities as explicit alternate observables, resolve the 23 literature repeats, and treat lensing and telluric caveats explicitly. Keep this luminous-quasar comparison family separate from faint JWST candidate populations.
...
\subsection*{3. Expand by coherent evidence family}
Use the generic batch gate for larger narrow-line, X-ray, photometric, lensed, high-ionization-line, and luminous-quasar additions. Each batch must share an evidence family, preserve source-level selection and mass-method distinctions, and stop for unresolved identity candidates. LRD remains a phenotype and lensing remains a property, not standalone object classes.

\subsection*{4. Build class-aware v7 science only after catalogue stability}
After at least the selected comparison batch is audited and frozen, produce separate measurement- and physical-object-level numeric products. Keep primary and exploratory tiers distinct, stratify unlike selections, leave the host-system view unranked, and audit tables before generating figures.

\subsection*{5. Improve the uncertainty model}
...
\subsection*{6. Generalize the first history diagnostics}
...
\subsection*{7. Convert the ranking into an observing strategy}
...
\begin{figure}[p]
\centering
\includegraphics[width=0.98\textwidth]{../results/releases/v2/figures/main_text/v2_main_text_spotlight_seed_redshift_maps.png}
...
The completed science work through v6 and the catalogue-only v7 assembly demonstrate the atlas concept. The repository combines provenance-aware ingestion, validated standardization, explicit measurement, object, and host identity, transparent physical modeling, point and Monte Carlo rankings, selection-aware summaries, duplicate-measurement sensitivity, class-aware eligibility, censored observables, and automated regression checks. v7 preserves every v1--v6 artifact and default while adding seven Ren candidate nuclei. Its full 119/112 catalogue remains available for exploratory diagnostics, while the 112/105 primary tier excludes six conditional masses. The source-family batch gate makes larger coherent additions practical without equating unlike selections or mass methods; XQR-30 is the next separate comparison batch. This is not evidence that one formation pathway is required. Class-aware v7 rankings remain future work until the expanded catalogue is stable.
...
\bibitem{ren2025}
Ren, J. et al. 2025, ``The ALPINE--CRISTAL--JWST Survey: Revealing Less Massive Black Holes in High-Redshift Galaxies,'' \emph{MNRAS}, 544, 211--233, doi:10.1093/mnras/staf1709.

\bibitem{mazzucchelli2023}
Mazzucchelli, C. et al. 2023, ``XQR-30: Black Hole Masses and Accretion Rates of 42 $z>6$ Quasars,'' \emph{A\&A}, 676, A71, doi:10.1051/0004-6361/202346317.
\end{thebibliography}
