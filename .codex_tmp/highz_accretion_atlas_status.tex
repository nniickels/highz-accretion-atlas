...
\large Project Status Through v4.0.1}
...
\begin{abstract}
JWST has identified accreting black-hole candidates at epochs where the available growth time is short and inferred physical quantities remain method-dependent. This project develops a reproducible, source-tracked, uncertainty-aware atlas that ranks observational leverage under explicit seed and accretion assumptions. Project v1 standardized 23 JADES broad-line AGN measurements at $z\geq4$; v2 added point and Monte Carlo rankings plus figure prototypes; v3 added 37 Taylor et al.\ CEERS/RUBIES measurements; and v4 adds 20 Matthee et al.\ EIGER/FRESCO and 16 Lin et al.\ ASPIRE measurements. The corrected v4 release has 96 measurements representing 94 physical objects, explicit reviewed identity decisions, separate detection-confidence and mass-reliability semantics, and one-at-a-time sensitivity tests for both multiply measured objects. Maintenance release v4.0.1 pins the software environment, adds continuous integration and a hash manifest, hardens future identity allocation, and records source-specific virial calibrations without changing a mass, rank, or scientific scenario. Under the baseline $z_{\rm seed}=30$, $\epsilon=0.1$, and no-merger reference, rankings express observational growth pressure and do not prove a unique seed or accretion channel. Next steps are final-manuscript consolidation, method-aware object classes, and duty-cycle diagnostics.
\end{abstract}
...
The corrected v4 point ranking keeps GN-38509 first and GS-20057765 second. RUBIES-EGS-49140 and GOODS-N-9771 rank third and fourth in physical growth pressure, but both are explicitly caveated for mass/line-model interpretation while retaining high detection confidence. Substituting CEERS-2782 for the default RUBIES-EGS-50052 measurement lowers the adopted mass by 0.42 dex and moves that physical object from growth-pressure rank 28 to 46; substituting Matthee GOODS-S-13971 for JADES GS-204851 lowers the mass by 0.19 dex and moves the object from rank 17 to 22. These are sensitivity results, not changes to release defaults.
...
Integrate the completed corrected-v4 figures and tables, verify every numeric claim and citation, and decide which source-stratified products belong in the main text or appendix.
...
The completed v1--v4 work demonstrates the atlas concept: provenance-aware ingestion, validated standardization, explicit reviewed measurement/object identity, transparent physical modeling, point and Monte Carlo rankings, selection-aware summaries, duplicate-measurement sensitivity, release-specific visualization, and automated regression checks. v4 broadens the lower-redshift end of the same-class BLAGN comparison while preserving every literature row and preventing double-counting. This is not evidence that one formation pathway is required. The next phase should finish the manuscript audit, then add carefully matched measurement-version layers, method-aware object classes, and more realistic accretion histories.
