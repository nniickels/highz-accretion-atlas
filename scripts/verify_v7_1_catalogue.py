"""Verify the catalogue-only v7.1 manifest and in-memory reproduction."""
...
MANIFEST_PATH = ROOT / "releases/v7.1-catalogue-manifest.json"
EXPECTED_SCOPE = "catalogue-only XQR-30 extension; no v7.1 science rankings or figures"
EXPECTED_PYTHON = "3.12"
EXPECTED_COUNTS = {
    "measurements": 161,
    "physical_objects": 154,
    "host_systems": 153,
    "growth_eligible_measurements": 160,
    "primary_measurements": 153,
    "growth_eligible_physical_objects": 153,
    "primary_physical_objects": 146,
    "measurement_object_links": 161,
    "object_host_links": 154,
    "aliases": 203,
    "reviewed_match_records": 0,
    "external_literature_identity_records": 23,
    "source_observables": 364,
    "xqr30_measurements": 42,
    "published_edd_ratio_inconsistencies": 4,
}
EXPECTED_SOURCE_ARCHIVES = {
    "arXiv:2306.16474v1": "412055cec92c368f711605822d806c949816695a451efee867904d2171fee53f",
    "arXiv:2305.05053v1": "1cf315f5fd4cd9f0edebb840c254dcd6bee26e2a061ce9fc9ff5bc8f344d7c42",
}
...
def observed_catalogue_counts() -> dict[str, int]:
    ...
    xqr30 = measurements["source_key"].eq("xqr30_mazzucchelli23")
    return {
        ...
        "external_literature_identity_records": len(frames["external_literature_identity_audit"]),
        "source_observables": len(frames["observables"]),
        "xqr30_measurements": int(xqr30.sum()),
        "published_edd_ratio_inconsistencies": int(
            (xqr30 & measurements["edd_ratio_consistency_flag"].eq("inconsistent")).sum()
        ),
    }
...
def verify_manifest_metadata(manifest: dict[str, object]) -> None:
    expected = {
        ...
        "parent_catalogue_release": "v7-accreting-atlas-catalogue",
...
def verify_manifest(manifest: dict[str, object]) -> None:
    ...
    if not isinstance(artifacts, dict):
        raise AssertionError("v7.1 catalogue manifest artifacts must be an object")
    ...
    if set(artifacts) != expected_paths:
        ...
        raise AssertionError(
            "v7.1 catalogue manifest membership mismatch:\n"
    ...
    if failures:
        raise AssertionError("v7.1 catalogue manifest hash mismatch:\n" + "\n".join(failures))
...
def main() -> None:
    ...
    if args.require_clean and worktree_status():
        raise AssertionError("v7.1 catalogue verification requires a clean Git worktree")
    ...
    if args.reproduce:
        verify_reproduction()
    if args.require_clean and worktree_status():
        raise AssertionError("v7.1 catalogue verification changed the Git worktree")
