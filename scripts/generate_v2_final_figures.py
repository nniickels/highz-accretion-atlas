"""Create v2 final-style figure prototypes from the v1 catalogue products.

The figures are main-text candidates driven by the v2 ranking and uncertainty
...
FIGURE_PATHS = {
    "growth_overview": FIGURE_DIR / "v2_main_text_mbh_redshift_growth_overview.png",
    "ranked_fedd": FIGURE_DIR / "v2_main_text_ranked_required_fedd.png",
    "ranked_seed": FIGURE_DIR / "v2_main_text_ranked_required_seed_mass.png",
    "pressure_confidence": FIGURE_DIR / "v2_main_text_pressure_vs_confidence.png",
    "uncertainty_forest": FIGURE_DIR / "v2_main_text_uncertainty_forest.png",
    "spotlight_maps": FIGURE_DIR / "v2_main_text_spotlight_seed_redshift_maps.png",
}
...
def read_ranking() -> pd.DataFrame:
    if not RANKING_PATH.exists():
        raise FileNotFoundError(
            f"Ranking table not found: {RANKING_PATH}. Run scripts/generate_v2_rankings.py first."
        )
...
def read_uncertainty_products() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if missing_paths:
        raise FileNotFoundError(
            ...
            f"{missing_paths}. Run scripts/generate_v2_uncertainty_rankings.py first."
        )
...
def main() -> None:
    ...
    print(f"Saved {len(paths)} final-style v2 figure prototypes from the v1 catalogue:")
    for path in paths:
