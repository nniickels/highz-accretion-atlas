"""Verify v5 hashes and optionally reproduce every current v5 CSV in memory."""
...
def _csv_round_trip(frame: pd.DataFrame) -> pd.DataFrame:
    """Match the on-disk dtype inference used by the normal science command."""
...
def verify_reproduction(manifest: dict[str, object]) -> None:
    ...
    science = build_science_outputs(
        ...
        measurements=_csv_round_trip(catalogue["measurements"]),
        objects=_csv_round_trip(catalogue["objects"]),
    )
