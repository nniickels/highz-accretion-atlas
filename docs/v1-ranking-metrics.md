# v1 Object-Ranking Metrics

This document defines the v1 ranking design for the observational atlas and the
implemented `results/v1_object_ranking_table.csv` table.
...
The v1 table should therefore keep physical metrics, robustness metrics, and
...
The default v1 ranking is measurement-level because the catalogue currently has
one row per source-paper measurement. A later physical-object table should group
multiple measurements through a stable `physical_object_id`.

## Baseline Assumptions

Unless a column name states otherwise, v1 ranking metrics use:
...
The proposed output file is `results/v1_object_ranking_table.csv`.
