...
The v7 source gate uses Ren et al., *The ALPINE--CRISTAL--JWST Survey:
...
explicit outflow/intermediate-width ambiguity. The v7 admission mapping is
therefore:
...
The source files and `src/v7_ren.py` pass the v7 admission gate in memory. This
extraction step itself creates no combined output; the subsequently verified
catalogue-only assembly is documented in `docs/v7-catalogue-schema.md`. No v7
science ranking or figure has been generated.
