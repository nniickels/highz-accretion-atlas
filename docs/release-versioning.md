...
They do not identify the version of a source paper. The current project release
is **v3**.
...
The v3 products supersede v1 for combined-catalogue work, but do not overwrite
or invalidate v1 or v2 artifacts.
...
- Use `project_version` for the standardization release (`v1` or `v3`) and
  `catalogue_release` for the named combined catalogue (`v3-blagn`).
