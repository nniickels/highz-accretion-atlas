...
to 112 comparable broad-line AGN; and v3 is the final JWST-identified
heterogeneous atlas of 142 measurements, 133 physical objects, and 132 host
systems. Every version has
the same baseline evaluation, ranking, 10,000-sample Monte Carlo uncertainty,
duty-cycle, compatibility, and visual products. In v3, 112 objects support
numerical growth inference and 21 remain visible as explicit no-inference
...
v3 & JWST-identified heterogeneous atlas & 142 & 133 & 132 & 112 \\
\bottomrule
\end{tabular}
\end{table}
...
catalogue contains 133 unique physical objects even though it retains 142
...
dimensions and margins. Broad-line AGN are purple and luminous quasars are yellow.}
\label{fig:tracks}
\end{figure}
...
72 reference curves in total. Seed mass uses color, $f_{\rm Edd}$ uses line
style, efficiency uses line width, and merger boost uses opacity; numerical
...
X-ray candidate is gray. It is an assumption comparison rather than a fit.
An
...
\caption{Monte Carlo summary for all 112 numerical v3 objects. Marker size
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{../../../results/v3/parameter_maps/fedd_mass_maps/v3_fedd_mass_map_hza-gn-38509.png}
...
under \repo{results/<version>/parameter\_maps/}.
...
values remain available for audit. The follow-up matrix retains all 133
objects, ranking 112 and leaving 21 explicitly unranked; the caveat summary
...
\begin{enumerate}[leftmargin=1.5em,itemsep=3pt]
  ...
  \item \repo{results/v3/parameter\_maps/fedd\_mass\_maps/} and
  \repo{results/v3/parameter\_maps/seedredshift\_mass\_maps/}: the full visual supplement.
...
$\mathrm{v1}<\mathrm{v2}<\mathrm{v3}$, the expected 23/112/133 object counts,
