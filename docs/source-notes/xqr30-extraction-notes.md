# XQR-30 extraction and admission notes

The v3 catalogue admits the complete 42-object E-XQR-30 table from
...
- `python -m src.internal.extract_xqr30_arxiv_tables MASS_ARCHIVE SAMPLE_ARCHIVE`
...
The coordinate/redshift search finds no candidate against the admitted atlas source set.
Section 4.4 of the mass paper identifies 23 objects with earlier measurements
in the wider literature, but those comparison measurements are not separate
rows in v3. `data/crossmatch/v3/v3_external_literature_identity_audit.csv` therefore
...
comparability group. v3 produces catalogue, identity, observable, strata, and
class-aware science products. It is not pooled with faint-JWST samples for
demographic inference; global ranks are navigation aids and class/method fields
remain explicit.
