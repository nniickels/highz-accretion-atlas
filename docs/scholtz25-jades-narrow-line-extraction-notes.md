...
The v7.4 catalogue adds every row at `z >= 4` in Scholtz et al. (2025),
...
line fluxes. JADES DR3 v3.1.3 supplies target coordinates.

The paper repeatedly reports 42 candidates, but its released `Table_sample.tex`
contains 41 rows. The atlas does not manufacture the missing record: 20 of the
41 tabulated rows meet `z >= 4`, and all 20 are admitted. This discrepancy is
...
Eddington mass; the v7.2 growth population stays frozen. JADES 8083 is manually
...
`candidate` for this multiply measured object, reducing the primary-object
count from 171 to 170 without changing its numeric mass or growth eligibility.
