...
from src.expanded_catalogue import (
    TAYLOR_MASS_METHOD,
    TAYLOR_PAPER_VERSION,
    TAYLOR_SOURCE_KEY,
    build_expanded_catalogues,
    standardize_taylor,
    validate_taylor_raw,
)
...
LINK_PATH = REPO_ROOT / "data" / "crossmatch" / "measurement_object_links.csv"
MEASUREMENT_OUTPUT_PATH = REPO_ROOT / "data" / "processed" / "expanded_blagn_measurements.csv"
OBJECT_OUTPUT_PATH = REPO_ROOT / "data" / "processed" / "expanded_blagn_objects.csv"
...
class ExpandedReleaseTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.v1 = pd.read_csv(V1_PROCESSED_PATH)
        cls.raw = pd.read_csv(TAYLOR_RAW_PATH)
        cls.links = pd.read_csv(LINK_PATH)
        cls.measurements, cls.objects = build_expanded_catalogues(cls.v1, cls.raw, cls.links)
